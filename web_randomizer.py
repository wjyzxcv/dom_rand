...
def dom_ran(mode, expan="0", n_cards="2"):
    ...
    if mode == "a":
        ...
        return pd_df2html(
            pd.DataFrame(
                np.random.choice(name_sets, n_req, replace=False),
                columns=["Expansions"],
            )
        )
    elif mode == "b":
        ...
        valid_exp = df_name_sets[
            df_name_sets["Set"].isin(
                ["Empires", "Adventures", "Menagerie", "Plunder", "Promo"]
            )
        ]
        set_ind = expan
        set_name = df_name_sets.iloc[set_ind, 0]
        ...
        n_traits = n_cards
        return pd_df2html(trait_table["Name"].to_frame().sample(n_traits))
    elif mode == "ini":
...
def index():
    ...
    if request.method == "POST":
        ...
        return render_template(
            ...
            outputs=outputs,
            expan_list=expan_list,
        )
    else:
        return render_template(
            "index.html",
            output_table="",
            outputs="Welcome to my crappy Dominion Randomizer!",
            expan_list=expan_list,
        )
